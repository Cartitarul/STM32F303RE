...
/* USER CODE BEGIN PV */
uint8_t A[5]={0x30,0x31,0x32,0x33,0x34};
...
void HAL_UART_RxCpltCallback(UART_HandleTypeDef *huart)
{
	...
	if(huart->Instance == USART2)
	{
		...
		if(rx_data[0] != 13){
			...
			cleanBuffer(data_buffer);
			cleanBuffer(cleaned_buffer);
			cleanBuffer(cleaned2buffer);
			//	for(i=0;i<100;i++){ 		// Cleaning Buffer after every read to not have garbage data stored.
			//		data_buffer[i]=0;		//
			//		cleaned_buffer[i]=0;	//
			//	}
			strcpy(data_buffer,rx_buffer);
			for (i = 0; i < strlen((char*)data_buffer); i++) {
			        if (data_buffer[i] != ' ') {
			           cleaned_buffer[j++]=data_buffer[i];
			        }
			    }
			j=0;
